...
window.addEventListener('DOMContentLoaded', (e) => {
  ...
  const startAt = repairsScroll.getAttribute('data-start-at');
  ...
  const coordinate = (y) => {
    if (startAt === '50%') {
      if (y === 1)
        return (
          (getCoords(repairsImgs[y]).top - getCoords(repairsScroll).top) / 2
        );
      return (
        ((getCoords(repairsImgs[y]).top - getCoords(repairsScroll).top) /
          (y + 2)) *
        (y + 1)
      );
    } else {
  ...
  const coordinateMobile = (y) =>
    getCoords(repairsImgs[y]).top +
    repairsImgs[y].offsetHeight / 2.2 -
    repairsImgs[y].offsetHeight * 2;
  const handleDesktopScroll = (e) => {
    ...
    for (let i = 0; i < repairsImgs.length; i++) {
      if (repairsImgs[i + 1]) {
        if (
          +(window.scrollY - getCoords(repairsScroll).top).toFixed() >
            coordinate(i) &&
          +(window.scrollY - getCoords(repairsScroll).top).toFixed() <
            coordinate(i + 1)
        ) {
